...
1. **將內容新增至各自位置的管道。**
   ...
   導覽至 **示範** > **位置** > **地區A** > **地區A** 並選取 **編輯** 從動作列移除。 拖放您要新增至頻道的資產。
   ...
   同樣地，導覽至 **示範** > **位置** > **區域B** > **區域B** 並選取 **編輯** 將資產拖放至您的頻道中，如下所示：
...
1. **建立排程**
   ...
   導覽並選取 **時程表** AEM Screens資料夾。 然後選取 **建立** 從動作列移除。
...
1. **將頻道指派至排程**
   ...
   1. 瀏覽至 **示範** > **時程表** > **AdSchedule** 並選取 **儀表板** 從動作列移除。
   1. 選取 **+指派管道** 從 **已指派的管道** 面板，讓您可以開啟 **頻道指定任務** 對話方塊。
   1. 選取 **引用頻道** 依路徑。
...
1. **指派排程給每個顯示器**
   ...
   1. 選取 **儀表板** 從動作列移除。
   1. 在控制面板上，選取 **...** 從 **已指派的頻道與排程** 面板，然後選取 **+指派排程**.
...
1. **建立群組**
   ...
   1. 選取 **工具** > **安全性** > **群組**.
   1. 選取 **建立群組** 並輸入 **Global-Author** 在 **ID**.
   1. 選取「**儲存並關閉**」。
...
1. **建立使用者並將使用者新增至群組**
   ...
   1. 瀏覽至 **Adobe Experience Manager**.
   1. 選取 **工具** > **安全性** > **使用者**.
   1. 選取 **建立使用者** 並輸入 **全域 — 使用者** 在 **ID**.
   1. 輸入 **密碼** 並確認此使用者的密碼。
   1. 選取 **群組** 標籤並在中輸入群組名稱 **選取群組**，例如，輸入 **Global-Author** 以新增 **全域 — 使用者** 至該特定群組。
   1. 選取「**儲存並關閉**」。
...
1. **存取每個群組的許可權**
   ...
   1. 搜尋 **Global-Author** 並選取 **許可權** 標籤，如下圖所示。
