...
description: 請依照本頁面的說明進行，瞭解如何在您的AEM Screens專案中實作動態內嵌序列。
...
***動態內嵌序列*** 是為遵循父項子階層的大型專案所建立，其中子項在位置資料夾內參照，而不是在管道資料夾內參照。 這可讓使用者透過以下方式將序列內嵌在管道中 ***頻道角色***. 它可讓使用者使用主頻道內的內嵌順序，為不同的辦公室定義位置特定的預留位置。

將管道指派給顯示時，您可以選擇指定顯示的路徑，或管道的角色來根據內容解析為實際管道。

若要使用「動態內嵌序列」，您必須透過以下方式指派管道 ***頻道角色***. 管道角色定義顯示的內容。 角色是各種動作的鎖定目標，與履行角色的實際管道無關。 本節說明依角色定義管道的使用案例，並說明如何將該內容用於全域管道。 您也可以將角色視為指派的識別碼，或前後關聯中管道的別名。

### 使用動態內嵌序列的優點 {#benefits-of-using-dynamic-embedded-sequences}

將順序頻道放在位置內而非頻道資料夾的主要優點，是允許本機或區域作者編輯與他們相關的內容，同時受限制無法編輯階層中較高位置的頻道。

引用 *依角色頻道*，可讓您建立本機版本的管道，以動態解析位置專用內容，也可讓您建立將內容用於位置專用管道的全域管道。
...
>
>***動態內嵌序列*** 可讓您顯示特定位置的內容，但 ***內嵌順序*** 只顯示內容的一般投影片放映。 此外，在設定動態內嵌序列時，您需要使用頻道角色和名稱來設定頻道。 如需實際實施，請參閱以下步驟。
>
...
開始實作此功能之前，請確定您已具備下列必要條件，可以開始實作動態內嵌序列：
...
1. **在每個位置下建立顯示和通道。**
   ...
   >在此範例中，位於下方的管道 **地區A** 和 **區域B** 共用相同 **名稱** 作為 **區域** 和管道位於 **商店1**， **商店2**， **商店3**、和 **商店4** 共用相同 **名稱** 作為 **儲存**.
   ...
   依照上述步驟建立管道並顯示在 **商店3** 和 **商店4** 在 **區域B**. 再次強調，請確定您使用 **名稱** 作為 **儲存** 建立管道時 **Store3Channel** 和 **Store4Channel** （分別）。
...
1. **建立排程**
   ...
   導覽並選取 **時程表** AEM Screens資料夾，然後按一下「 」 **建立** 以建立新的排程。
...
1. **將頻道指派至排程**
   ...
   1. 按一下 **+指派管道** 從 **已指派的管道** 面板以開啟 **頻道指定任務** 對話方塊。
   1. 選取 **引用頻道**.. 依路徑。
...
>[!NOTE]
>
>要詳細瞭解如何使用ACL來分離專案，以便每個人或團隊處理自己的專案，請參閱 **設定ACL**.
...
1. **修改每個群組的許可權**
   ...
   **若為Global-Author：**
   ...
   下圖顯示現在 **全域 — 使用者** 可以存取 **全域頻道** 和兩者 **地區A** 和 **區域B** 所有四個店舖 **商店1**， **商店2**， **商店3**、和 **商店4**.
   ...
   **若為Region-Author：**
   ...
   下圖顯示，現在Region-User可以同時存取 **地區A** 和 **區域B** 所有四個店舖 **商店1**， **商店2**， **商店3**、和 **商店4** 但無法存取 **全域** 頻道。
   ...
   **對於Store-Author：**
   ...
   下圖顯示現在 **Store-User** 只能存取四個存放區，即 **商店1**， **商店2**， **商店3**、和 **商店4** 但沒有許可權存取 **全域** 或地區(**地區A** 和 **區域B**)管道。
