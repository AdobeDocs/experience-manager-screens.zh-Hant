...
若要使用「動態內嵌序列」，您必須透過以下方式指派管道 ***頻道角色***. 管道角色定義顯示的內容。 角色以各種動作為目標，與履行角色的實際管道無關。 本節說明依角色定義管道的使用案例，並說明如何將該內容套用至全域管道。 您也可以將角色視為指派的識別碼，或前後關聯中管道的別名。

### 使用動態內嵌序列的優點 {#benefits-of-using-dynamic-embedded-sequences}

將順序頻道放在位置而非頻道資料夾中，可讓本機或區域作者編輯與他們相關的內容。 它也能限制您編輯階層中較高位置的管道。

引用 *依角色頻道*，可讓您建立頻道的本機版本。 這樣做會動態解析特定於位置的內容，並可讓您建立將內容用於特定於位置的管道的全域管道。
...
>[!NOTE]
>
>若要深入瞭解頻道指定任務，請參閱 **[頻道指定任務](channel-assignment.md)** 在AEM Screens檔案的「編寫」區段底下。
...
1. **在每個位置資料夾下建立兩個位置和一個頻道。**
   ...
   1. 瀏覽至 **示範** > **位置** > **地區A**.
   1. 按一下 **地區A** 並按一下 **+建立** 從動作列移除。
   1. 按一下 **位置** 從精靈，使用 **標題** 作為 **商店1**. 同樣地，從精靈中建立另一個名為 **商店2** 使用 **標題** 作為 **商店2**. 您可以離開 **名稱** 建立時欄位空白 **商店1** 和 **商店2**.
   1. 重複步驟(b)，現在按一下 **順序頻道** 從精靈中。 輸入 **標題** 作為 **地區A** 和 **名稱** 作為 **區域** 用於此管道。

   >[!CAUTION]
   >
   >建立管道時請務必確認 **地區A**，輸入 **標題** 作為 **地區A** 和 **名稱** as a **區域**.
...
1. **在每個位置下建立顯示和通道。**
   ...
   1. 瀏覽至 **示範** > **位置** > **地區A** > **商店1**.
   1. 按一下 **商店1** 並按一下 **+建立** 從動作列移除。
   1. 按一下 **顯示** 並從精靈建立 **`Store1Display`**.
   1. 重複步驟(b)，這次按一下 **順序頻道** 從精靈中。 輸入 **標題** 作為 **`Store1Channel`** 和 **名稱** 作為 **儲存**.
...
1. **建立排程**
   ...
   瀏覽並按一下 **時程表** AEM Screens資料夾。 然後按一下 **建立** 從動作列移除。
...
1. **將頻道指派至排程**
   ...
   1. 輸入 **頻道角色**，就像 **GlobalAdSegment**.
   1. 按一下 **支援的事件**，就像 **初始載入**， **閒置畫面**、和 **使用者互動**.
   1. 按一下「**儲存**」。

   **依地區角色指派頻道：**

   1. 按一下 **+指派管道** 從 **已指派的管道** 面板。
   1. 在「頻道指定任務」對話方塊中，按一下 **引用頻道** 依名稱。
   1. 輸入 **頻道名稱** 作為 **區域***.
...
1. **指派排程給每個顯示器**
   ...
   1. 按一下排程的路徑(例如， **示範** > **時程表** > **AdSchedule**)。
   1. 按一下「**儲存**」。
...
1. **建立使用者並將使用者新增至群組**
   ...
   1. 輸入 **密碼** 並確認此使用者的密碼。
   1. 按一下 **群組** 標籤並在中輸入群組名稱 **按一下群組**，例如，輸入 **Global-Author** 以新增 **全域 — 使用者** 至該特定群組。
   1. 按一下&#x200B;**「儲存並關閉」**。

   同樣地，建立兩個其他使用者，例如 **Region-User** 和 **Store-User**，並將它們新增至 **Region-Author** 和 **Store-Author** （分別）。
...
1. **將所有群組新增至貢獻者**
   ...
   1. 瀏覽至 **Adobe Experience Manager**.
   1. 按一下 **工具** > **安全性** > **群組**.
   1. 按一下 **貢獻者** 從清單中按一下 **成員** 標籤。
   1. 按一下 **群組** 例如 **Global-Author**， **Region-Author，** 和 **Store-Author** 貢獻者。
   1. 按一下&#x200B;**「儲存並關閉」**。

1. **存取每個群組的許可權**

   1. 導覽至 *使用者管理員* 和此UI來修改不同群組的許可權。
...
1. **修改每個群組的許可權**
   ...
   **若為Global-Author：**
   ...
   以下顯示 **全域 — 使用者** 可以存取 **全域頻道**. 以及，存取兩者 **地區A** 和 **區域B** 與全部四家店舖 **商店1**， **商店2**， **商店3**、和 **商店4**.
   ...
   **若為Region-Author：**
   ...
   以下顯示Region-User可以存取這兩者 **地區A** 和 **區域B**. 此外，亦可存取全部四家店舖，即 **商店1**， **商店2**， **商店3**、和 **商店4**，但不存取 **全域** 頻道。
   ...
   **對於Store-Author：**
   ...
   以下顯示 **Store-User** 只能存取 **商店1**， **商店2**， **商店3**、和 **商店4**. 但是，它沒有許可權存取 **全域** 或地區(**地區A** 和 **區域B**)管道。
