...
1. 選取您要內嵌頁面的管道。 例如， **`We.Retail`店內** > **頻道** > **閒置頻道**.

1. 選取 **編輯** 從動作列移除。
1. 在編輯器模式中，從左側欄選取元件圖示，即可新增內嵌頁面。 拖放 **內嵌順序** 至編輯器。
1. 按兩下 **內嵌順序** 元件，以便您將頻道新增到原始序列頻道。
1. 選取 **頻道路徑** 頻道的ID。
1. 選取 **持續時間（毫秒）** ，此專案適用於您在中的內嵌管道 **序列** 標籤。 根據預設，持續時間設為 **-1**，表示內嵌管道已完整執行。 如果使用者指定持續時間，則次序列會在指定時間中斷（即切斷）。
...
>1. 選取 **儀表板** 從動作列移除。
>1. 在顯示控制面板上，選取 **+指派管道** 從 **已指派的管道和已排程的面板** 以便您開啟 **頻道指定任務對話方塊**.
>
>1. 選取您（用於內嵌順序）中所使用管道的路徑 **頻道路徑**.
>1. 確定 **優先順序** 低於主要管道。
>
>1. 不要選取任何 **支援的事件**.
>1. 選取 **儲存** 完成時。
...
1. 選取您要內嵌動態序列的管道。 例如， **`We.Retail`店內** > **頻道** > **閒置頻道**.

1. 選取 **編輯** 從動作列移除。
1. 在編輯器模式中，從左側欄選取元件圖示，即可新增動態內嵌序列。 拖放 **動態** **內嵌順序** 至編輯器。
