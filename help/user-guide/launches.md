...
如果上線日期為 *11月24日，上午9點* 和 *24小時* 是寬限期，則上述動作序列會在（上線日期 — 寬限期） （即11月23日伺服器時間上午9:00）開始。 此設定提供24小時完成上述所有動作，讓內容可到達播放器。 播放器瞭解此期間是啟動內容。 因此，內容不會立即播放，但播放器可將此內容儲存為未來版本，並讓內容在播放器時區的設定上線日期完全開始播放。
...
>[!NOTE]
>
>現成可用的Screens Launch寬限期設為24小時。 這表示當您為底下的資源設定任何啟動的生效日期時 */content/screens*，促銷活動會從此位移開始。
...
您在上一個範例中將寬限期設為10分鐘。 因此，當您為下的資源設定任何啟動的生效日期時 */content/screens*，促銷活動會從此位移開始。
...
1. 按一下 **下一個** 從 **建立啟動項** 精靈。 此 **包含子頁面** 選項預設為選取。
   ...
   若要使用 **新增頻道** 選項，導覽至您要建立啟動項的頻道，然後按一下 **選取**.
...
* **生產就緒**  — 促銷活動後，可發佈管道並啟用現成可用功能，因此不需要變更。
...
您可以使用刪除啟動 **刪除啟動項** 選項來自 **啟動專案等待處理中** 面板。
