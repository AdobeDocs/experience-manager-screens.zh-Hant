...
在的協助下 ***畫面啟動***，作者可預覽啟動項中的每個管道，且應能起始檢閱請求。 核准者群組將收到通知，可以核准或拒絕請求。 當到達上線日期時，內容會在裝置播放。

例如，如果作者想要建立未來版本的c1、c2 （頻道），則會建立啟動並設定上線日期（例如，11月10日上午8:00）。 內容中的任何進一步更新都會送出以供檢閱。
...
* 將資源發佈到發佈例項（通常需要幾分鐘的時間，視需要發佈的管道或資產大小而定）

* 更新離線內容完成所花費的時間（通常需要幾分鐘）

* 播放器從發佈例項下載內容所花的時間（通常視需要下載的資產頻寬和大小而定，需要幾分鐘的時間）

* 伺服器和播放器的任何時間差異

### 瞭解寬限期 {#understanding-grace-period}

為了讓播放器能在設定的上線日期開始播放內容，我們需要在上線日期之前開始前面的活動。

如果上線日期為 *11月24日，上午9:00* 寬限期為 *24小時*，則上述動作順序將於（上線日期 — 寬限期）開始，亦即11月23日上午9:00的伺服器時間。 這可提供24小時時間完成上述所有動作，且內容會傳送至播放器。 播放器會瞭解這是啟動內容，因此不會立即播放內容，但播放器會將此內容儲存為未來版本，並完全在播放器時區的設定上線日期開始播放。

舉例來說，伺服器位於PST，而裝置位於EST，在此情況下，最大時間差異為3小時，並假設促銷活動需要1分鐘，而從作者發佈至發佈內容需要10分鐘，且播放器通常可在10-15分鐘後下載資源。 然後寬限期=時間差異（3小時） +提升啟動項的時間（1分鐘） +發佈啟動項的時間（10分鐘） +在播放器下載的時間（10-15分鐘） +緩衝區（以安全為例，30分鐘） = 3小時56分鐘= 14160秒。

因此，每當我們將任何啟動項排程為即時啟動時，促銷活動都會在此位移處提早開始。 在上述方程式中，大部分專案不需要花費太多時間，只要知道伺服器和任何播放器之間的最大時間差異，我們就可以針對此位移使用適當的猜測。

>[!NOTE]
>
>現成可用的Screens Launch的寬限期設為24小時，這表示當我們針對底下的資源設定任何啟動專案的上線日期時 */content/screens*，促銷活動會從此位移開始。
...
2. 按一下滑鼠右鍵並複製檔案。
3. 瀏覽至 `/apps/system/config` 然後按一下右鍵並貼上。
...
由於您已將上個範例中的寬限期設為10分鐘，因此當您為下的資源設定任何啟動專案的上線日期時 */content/screens*，促銷活動會從此位移開始。

例如，如果上線日期設定為11月24日上午9:00，且寬限期為600秒，則促銷工作將於11月24日上午8:50開始。
...
1. 輸入 **啟動項標題** 作為 **SummerPromotions** 而且您不需要設定 **啟動日期**，如下圖所示。 按一下&#x200B;**建立**。
   ...
   * **推廣已核准的頁面**：此選項需要在啟動管道上執行啟動核准工作流程。 只有已核准的頁面會在設定的上線日期提升。
...
* **啟動日期**，是指線上日期，也就是內容根據播放器時區在Screens播放器中播放的日期或時間。
* **生產就緒**，可在提升管道後發佈管道，這將會啟用立即可用功能，因此不需要變更。
* **範圍**，決定啟動項促銷期間要促銷的管道。


請依照下列步驟編輯啟動項屬性：

1. 導覽至該管道 **Futurelaunch**， *（即待處理的啟動）* 並選取通道，如下圖所示。
...
您可以使用手動提升啟動 **提升啟動** 選項來自 **啟動專案等待處理中** 面板。
...
1. 您可以設定 **範圍** ，並提供下列選項：
   ...
   1. **推廣已核准的頁面**：此選項需要在啟動管道上執行啟動核准工作流程。 只有已核准的頁面會在設定的上線日期提升。
   1. **提升目前頁面**：此選項會要求啟動核准工作流程只為目前頁面執行。
1. 按一下 **下一個** 在 **提升啟動** 精靈。
1. 按一下 **提升** 以提升啟動。

### 刪除畫面啟動項 {#deleting-the-screens-launch}
