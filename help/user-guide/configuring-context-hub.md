...
description: 請依照本頁面瞭解定位引擎中的ContextHub，以定義資料存放區，用於資料觸發內容變更。
...
在我們詳細瞭解如何在您的AEM Screens專案中建立和管理庫存導向管道之前，您必須瞭解一些對於不同情境重要且相關的關鍵術語。

**品牌** 請參閱您的高階專案說明。

**區域** 是指您的AEM Screens專案名稱，例如數位廣告招牌

**活動** 定義規則類別，例如，詳細目錄導向、天氣導向、部門可用性導向等。

**對象** 定義規則。

**區段** 指根據特定規則要播放的資產版本，例如如果溫度低於華氏50度，則畫面會顯示熱咖啡的影像，否則會顯示冷飲。
...
開始為AEM Screens專案設定Context Hub設定前，您必須先設定Google工作表（以供示範之用）。
...
>[!NOTE]
>
>以下特定範例將Google工作表示範為資料存放區，如果值大於100或小於50，則會觸發資產變更。
...
1. **建立新的ContextHub存放區組態**
   ...
   1. 從 **ContextHub設定** 熒幕，按一下 **建立** > **ContentHub存放區設定……**
      ...
      >1. 按一下右鍵 `/conf/screens/settings` 並透過建立新節點 *名稱*  作為 **cloudsettings1** 和 *型別* 作為 **sling：Folder** 並儲存變更。
   ...
   1. 輸入您專屬的json設定。 例如，您可以將以下json用於示範目的，然後按一下 **儲存** 而且您會看到標題為 **Google工作表** 在ContextHub設定中。
      ...
      >[!CAUTION]
      >
      如果您在全域資料夾之外（例如在您自己的專案資料夾中）建立Google Sheets存放區設定，則無法立即使用目標定位。

1. **設定商店分段**

   1. 瀏覽至 **ContentHub存放區設定……** 和在screens設定容器中建立另一個商店設定，並設定 **標題** 作為 **segmentation-contexthub**， **存放區名稱** 作為 **細分** 和 **存放區型別** 作為 **aem.segmentation**.
...
1. **在活動中建立區段**
   ...
   1. 輸入 **標題** 作為 **ValueCheck50** 和 **名稱** 作為 **valuecheck50**. 選取 **定位引擎** 作為 **ContextHub (AEM)** 從下拉式清單，然後按一下 **下一個**.

      ![影像](/help/user-guide/assets/context-hub/context-hub14.png)

   1. 按一下 **新增體驗** 從 **設定活動精靈**.

   1. 從 **受眾**，選取 **Higherthan50** 並按一下 **新增體驗** 並輸入 **標題** 作為 **高於50** **名稱** 作為 **高於50**. 按一下 **確定**.
...
1. **編輯區段**
   ...
   1. 選取區段 **Higherthan50**，然後按一下 **編輯** 從動作列移除。
   ...
   1. 選取 **谷歌眼表/value/1/0** 從的下拉式清單 **屬性名稱**.
      ...
      >[!NOTE]
此 **谷歌眼表/value/1/0** 請參考下圖中在google工作表中填入的列2和欄：
