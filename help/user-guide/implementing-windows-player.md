...
description: 請依照本頁瞭解如何設定AEM ScreensWindows 10播放器。
seo-description: 請依照本頁瞭解如何設定AEM ScreensWindows 10播放器。
...
本節介紹配置AEM ScreensWindows 10播放器。 它提供設定檔、可用選項和建議的資訊，說明要用於開發和測試的設定。
...
>[!NOTE]
>如果您使用AEM Screens6.5.5 Service Pack，則必須為Windows Player設定環境。
...
1. 導覽至&#x200B;**Adobe Experience ManagerWeb控制台
...
臨機方法可讓您安裝最新的Windows Player(*.exe*)。 請造訪&#x200B;[**AEM 6.5播放器下載**](https://download.macromedia.com/screens/)頁面。
...
| **原則名稱** | **目的** |
|---|---|
| 伺服器 | 指向Adobe Experience Manager(AEM)伺服器的URL。 |
| 解析度 | 裝置的解析度。 |
...
1. 從[AEM Screens播放器下載](https://download.macromedia.com/screens/)頁面為該Kiosk用戶安裝windows播放器。
...
***C:\Users\&amp;lt;your user>\AppData\Local\Programs\@aem-screensscreens-player-electron\AEM ScreensPlayer.exe***
