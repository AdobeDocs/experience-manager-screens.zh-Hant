...
>[!NOTE]
>
>請參閱「編寫畫面」區段下的[內嵌序列](embedded-sequences.md)，以詳細瞭解內嵌序列。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>將其設為<strong>original</strong>或<strong>single</strong>。 將值設為<strong>original</strong>表示子序列將在父序列的每個循環上完全運行。 另一個可能的值是<strong>single</strong>，且每個執行只會顯示一個子系項目（例如，第一個循環上的第一個項目、第二個循環上的第二個項目等）。</td>
  </tr>
 </tbody>
</table>
...
>[!NOTE]
>
>從此元件添加的&#x200B;**客戶端庫**&#x200B;和&#x200B;**靜態檔案**&#x200B;除了已配置的&#x200B;**客戶端庫**&#x200B;和從體驗片段的&#x200B;**屬性**&#x200B;添加的靜態檔案外。

### 影像 {#image}

「影像」可讓您將影像新增至頻道。

該影像資產具有三個頁籤，即&#x200B;**Image**、**Accessibility**&#x200B;和&#x200B;**Sequence**:
...
| ***替代文字*** | 影像的替代文字。 |
| **順序** |
| ***持續時間*** | 預設情況下，持續時間設定為&#x200B;*8000 ms*。 如果要更改影像的播放持續時間，請更新&#x200B;**持續時間**&#x200B;欄位。 |
...
選取轉場圖示，然後按一下「設定&#x200B;****（扳手圖示）」以開啟「轉場&#x200B;****」對話方塊。 此對話方塊包含三個標籤：
...
>[!NOTE]
>
>依預設，序列設定為600毫秒。 您可以使用&#x200B;**序列**&#x200B;標籤將轉換序列更新為其他值。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td><p>元素之前和之後之間的過渡類型。 轉換<strong>Type</strong>包含下列選項：</p>
    <ul>
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>選擇視訊的持續時間。 依預設，持續時間會設為-1，表示元素會永遠執行。 設定持續時間值&gt;0，顯示指定持續時間的元素，然後移至下一個。<br /> </td>
  </tr>
  <tr>
   <td><em><strong>轉譯</strong></em></td>
   <td><p>如果視訊高寬比不符合螢幕，您可將演算調整為<strong>contain</strong>或<strong>cover</strong>。</p> <p><em>包</em> 含：會顯示完整視訊，而遺失的區域會填入黑邊。</p> <p><em>Cover</em> 意指視訊涵蓋整個視訊區，但溢位於兩側的部分會隱藏。</p> </td>
  </tr>
  <tr>
