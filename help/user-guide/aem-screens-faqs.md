...
### &#x200B;1. 面對黑熒幕或未播放內容的客戶，應採用哪些急救疑難排解步驟？ {#troubleshooting-blank-screen}
...
* 檢查播放器主控台記錄檔，並檢查是否有任何錯誤。 按一下滑鼠右鍵並檢查以檢視主控台記錄檔。 如果您使用Windows Player，請按`CTRL + ALT +I`開啟開發主控台，以檢視記錄檔。
...
### &#x200B;1. 線上和離線管道有何差異？ {#what-is-the-difference-between-an-online-and-an-offline-channel}
...
### &#x200B;3. 「頻道角色」欄位有何用處？ {#what-is-the-use-of-the-channel-role-field}

管道角色是實際執行管道的抽象概念，可讓作者直接專注於一般體驗。 您可以將之視為可在頻道內容（顯示或排程）中唯一識別該頻道的標籤種類。

### &#x200B;4. 如何實際解析頻道？ {#how-does-actual-channel-resolution-happen}
...
### &#x200B;2. 如果AEM Screens Player無法註冊且其狀態顯示為「失敗」，怎麼辦？
...
### &#x200B;3. 註冊AEM Screens Player時，若裝置顯示失敗且主控台記錄檔顯示ENAME_NOT_FOUND錯誤，該如何進行疑難排解？

如果播放器找不到AEM Screens伺服器DNS，就可能會發生此問題。 您可以嘗試使用IP位址進行連線。 若要取得伺服器的IP，請使用： *arp &lt;server_dns_name>*。

### &#x200B;4. AMS建議在所有裝置上實作™監視程式嗎？ APK是否包含Watchdog (Cordova)外掛程式？ {#does-ams-recommend-implementing-an-android-watchdog-on-all-devices-is-the-watchdog-cordova-plugin-included-as-part-of-the-apk}

使用純™ API的跨平台Android™監視程式已經是apk的一部分。 不需要其他軟體。 不過，視您使用的裝置而定，您可以視需要放棄apk以取得完整電源週期(`Powermanager` api)的系統許可權。 如果它沒有使用製造商金鑰辭職，它會退出並重新啟動應用程式，但不會重新啟動電源。

如需如何實作™ Player的詳細資訊，請參閱&#x200B;[**實作Android™ Player**](implementing-android-player.md)。

### &#x200B;5. Adobe/AMS建議使用哪些協力廠商遠端監控和警報工具（軟體）來監控每個裝置？ {#what-third-party-remote-monitoring-and-alerting-tools-software-does-adobe-ams-recommend-for-monitoring-each-device}
...
### &#x200B;3. 如何在™ Player中設定裝置熒幕持續顯示？ {#how-to-set-the-device-screen-stay-on-in-an-android-player}

請依照下列步驟，在任何™播放器上開啟保持醒來功能：
...
1. 導覽至 `http://localhost:4502/system/console/configMgr`
1. Day CQ連結外部化器
