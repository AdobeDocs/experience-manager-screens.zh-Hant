...
1. 取得本機HTTP伺服器的IP或URL （以及步驟2中包含已擷取檔案之資料夾的路徑，前提是已擷取至子資料夾而非根資料夾）
...
1. 按一下遠端上的功能表按鈕。
1. 瀏覽至 **網路** > **裝置名稱** 以指派名稱給播放器。

### 在Samsung裝置上設定更新 {#config-updates}

在Samsung裝置上依照下列步驟操作，完成裝置上的AEM Screens播放器安裝：
...
1. 設定URL啟動器後，按下 **首頁** 遙控器上的按鈕。

1. 導覽至 **URL啟動器設定** 並輸入localhost伺服器的IP位址，然後按一下 **完成**.
   >[!NOTE]
   >Tizen播放器應能連線至http伺服器。

1. AEM Screens Player現在應該會自動在您的Samsung裝置上安裝並啟動。
...
>有些瀏覽器引擎與 *SameSite=None* 用於AEM 6.5核發至AEM 6.7的登入權杖中的屬性。通常，將瀏覽器升級至最新可用版本即可解決此問題。 在某些情況下，可能無法進行這類升級，例如使用智慧型顯示器、機上盒或其他內嵌瀏覽引擎的裝置。
...
1. 您應該會看到新選項 *使用者代理將免除samesite屬性的限制*. 將對應至使用者代理程式（與）不相容的規則運算式填入此運算式 *SameSite=None* 屬性。
   >[!NOTE]
...
從遠端布建Tizen Player，您無需費力就能部署成百上千的Samsung Tizen顯示器。 它可避免使用伺服器URL和大量註冊代碼或其他引數設定每個播放器，以及在Screensas a Cloud Service設定雲端模式和雲端權杖的情況下繁複的手動工作。

此功能可讓您從遠端設定Tizen播放器，並視需要集中更新這些設定。 您只需要使用 `HTTP` 用來裝載Tizen應用程式的伺服器 `(wgt and xml file)` 以及文字編輯器，以儲存 `config.json` 搭配適當的引數。

請確定您已在Tizen裝置上設定URL啟動器位址，也就是「首頁」按鈕> URL啟動器設定。
在 `HTTP` 裝載Tizen應用程式的伺服器，放置檔案 `config.json` 與位於相同位置 `wgt` 檔案。 檔案名稱必須是 `config.json`.
Tizen播放器將會安裝，並在啟動時（以及每次重新開機）檢查並套用中的設定。 `config.json` 檔案。
...
>[!NOTE]
>原則設定會嚴格執行，且不會在播放器的管理員UI中手動覆寫。 若要允許特定原則的手動播放器設定，請勿在原則設定中指定原則，例如，如果您要允許重新開機排程的手動設定，請勿指定索引鍵 `rebootSchedule` 在原則設定中。 每次播放器重新載入時都會讀取原則設定。
...
1. 設定TLS （如果需要）。 瀏覽至連線埠，並從伺服器選取連線埠號碼，然後按一下 **儲存**.

1. 導覽至 **裝置** 標籤並檢查您剛設定的裝置。 找到裝置後，按一下核取方塊並選取 **核准**.

   >![影像](/help/user-guide/assets/tizen/rms-3.png)

1. 填寫所需資訊，然後選取裝置群組。 按一下 **確定** 以完成核准程式。

   >![影像](/help/user-guide/assets/tizen/rms-7.png)

1. 裝置獲得核准後，應會顯示在「裝置清單」上。 按一下 *資訊* 按鈕位於您的裝置方塊上，也就是 **ì**，如下圖所示。
...
1. 編輯裝置選項並選取 **設定** 標籤。 瀏覽至 **URL啟動器** 區段，並輸入託管wgt的URL和 `SSSP config file` 安裝 `SSSP` 應用程式，如下圖所示。
