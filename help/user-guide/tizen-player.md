...
1. AEM重新啟動後，移至`/system/console/configMgr`並搜尋&#x200B;**AdobeGranite權杖驗證處理常式**。 將&#x200B;**SameSite**&#x200B;的值設定為&#x200B;**無**。
...
從遠端布建Tizen播放器，讓您輕鬆部署成百上千的Samsung Tizen顯示器。 這樣可以避免手動使用伺服器URL和大量註冊代碼或其他引數來設定每個播放器。 如果有AEM Screensas a Cloud Service，則需設定雲端模式和雲端代號。
...
| cloudMode | 如果您希望Tizen播放器連線至Screensas a Cloud Service，請設為true。 設為false可連線至AMS或內部部署AEM。 |
