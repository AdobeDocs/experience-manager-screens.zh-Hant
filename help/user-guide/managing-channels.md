...
description: 請詳閱本頁面，瞭解如何建立和管理管道。 此外也說明管道控制面板和編輯管道的內容。
...
為AEM Screens建立專案後，請依照下列步驟為專案建立新的管道：

1. 選取Adobe Experience Manager連結（左上方），然後選取Screens。 或者，您可以直接導覽至 `https://localhost:4502/screens.html/content/screens`.

1. 導覽至您的畫面專案，然後選取 **頻道** 資料夾。

1. 按一下 **建立** 從動作列移除。

   ![demochannel](assets/create-channel1.png)

1. 選取 **順序頻道** 範本來自 **建立** 精靈並按一下 **下一個**.
...
| 應用程式頻道 | 允許在Screens播放器中顯示您的自訂Web應用程式。 |
...
>[!NOTE]
>
>「拆分畫面」頻道會將顯示分割成多個區域，以便您同時並排播放數個體驗。 體驗可以是靜態資產/文字或內嵌序列。

>[!IMPORTANT]
>
> 建立內容並新增至管道後，下一步就是建立位置，然後建立顯示。 此外，您需要將該頻道指派給顯示器。 請參閱以下章節結尾的資源，瞭解更多資訊。
...
1. 選取您要編輯的管道（如上圖所示）。
1. 按一下 **編輯** 從動作列的左上角編輯色版屬性。 編輯器會開啟，讓您新增資產/元件至您要發佈的管道。
...
1. 選取您要編輯的管道。
1. 按一下 **儀表板** 以檢視控制面板。 此 **頻道資訊**，**指派的顯示區**、和 **啟動專案等待處理中** 面板會開啟，如下圖所示：
...
按一下(**...**)從 **頻道資訊** 動作列來檢視屬性、編輯內容，或更新頻道的快取（離線內容）。
